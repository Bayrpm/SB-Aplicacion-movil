...
const BASE_ITEM_WIDTH = Math.round(
  Math.min(Math.max(WINDOW_WIDTH * 0.34, 140), 220)
);
const ITEM_WIDTH = Math.round(BASE_ITEM_WIDTH * 0.9); // -10% ancho
const ITEM_HEIGHT = Math.round(BASE_ITEM_WIDTH * 0.9 * 1.05 * 1.1); // +5% y +10% alto
...
const getLogo = (id?: string) => {
  switch (id) {
    case "carabineros":
      return require("@/assets/images/carabineros.png");
    case "samu":
      return require("@/assets/images/samu.png");
    case "bomberos":
      return require("@/assets/images/bomberos.png");
    // Temporarily use a safe fallback image while we fix the original pdi asset
    case "pdi":
      return require("@/assets/images/pdi.png");
    case "seguridad":
      return require("@/assets/images/seguridad.png");
    default:
      return require("@/assets/images/icon.png");
  }
};

const logoScaleFor = (id?: string) =>
  id === "seguridad" ? 2.2 : id === "bomberos" ? 1.1 : 1.0;
...
export default function EmergencyCarousel({ items }: { items: Item[] }) {
  ...
  const renderItem = ({ item }: { item: Item }) => {
    const palette = PALETTE[item.id] ?? {
      bg: scheme === "dark" ? "#0b2030" : "#f7fbff",
      num: "#0A4A90",
    };

    const NUMBER_FS = Math.max(22, Math.min(64, Math.round(ITEM_HEIGHT * 0.3)));
    ...
    const logoBoxH = Math.max(
      40,
      contentH - numberAreaH - LOGO_MB + EXTRA_LOGO
    );

    const desired = Math.round(logoBoxH * logoScaleFor(item.id));
    const imageSize = Math.max(
      36,
      Math.min(logoMaxFor(item.id), logoBoxH, desired)
    );
    const finalImage =
      item.id === "seguridad"
      ...
      <Pressable onPress={onCall} style={{ marginHorizontal: ITEM_SPACING }}>
        ...
        >
          ...
          >
            <Image
              ...
              style={{
                ...
                height: finalImage,
                transform:
                  item.id === "seguridad" ? [{ scale: 1.45 }] : undefined,
              }}
              contentFit="contain"
            />
          </View>
          ...
          <Text
            style={[
              styles.itemNumber,
              { color: palette.num, fontSize: NUMBER_FS },
            ]}
            numberOfLines={1}
